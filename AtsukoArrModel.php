...
class AkbArr{
    ...
    public function where($where){
        if(is_string($where)){
            ...
            if(preg_match('/ or /i',$where)){
                $whereall=preg_split('/ or /i',$where);
                $stor=$this->arr;
                $return=[];
                ...
                $this->uniq();
                return $this;
            }elseif(preg_match('/ and /i',$where)){
                $whereall=preg_split('/ and /i',$where);
                foreach ($whereall as $v) {
                    $this->where($v)->select();
                }
                return $this;
    ...
    public function limit($limit){
        if(is_string($limit)&&preg_match('/(.*),(.*)/',$limit,$newlimit)){
            $this->arr=array_slice($this->arr,$newlimit[1]-1,$newlimit[2]);
        }else{
    ...
    public function vkSort($arr=false,$desc='v desc,k desc'){
        if($arr===false){
            return $this;
        }elseif(is_array($arr)&&(count($arr)==count($arr,1))){
            $big=[];
            foreach ($arr as $k => $v) {
                $small=[];
                $small['k']=$k;
    ...
    public function valueToKey($key){
        $stor=[];
        foreach($this->arr as $v){
            $stor[$v[$key]][]=$v;
        }
